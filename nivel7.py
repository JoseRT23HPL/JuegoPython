...
try:
    ...
    nivel_music = "sound/f7.mp3"  # Cambia por tu canción de nivel
...
class FlashEffect:
    def __init__(self):
        ...
        self.duration = 0.3
        self.flash_alpha = 0
        self.dark_alpha = 180  # Menos oscuro para poder ver algo
        self.dark_mode = True  # Empieza en modo oscuro
        self.dark_timer = 0
        self.dark_duration = 5.0
        self.light_duration = 3.0
        self.cycle_timer = 0
        self.initial_darkness = True  # Oscuridad inicial hasta segundo 20
        self.initial_dark_timer = 0
        self.initial_dark_duration = 21.0  # 20 segundos de oscuridad inicial
    ...
    def update(self, dt):
        ...
        if self.initial_darkness:
            ...
            if self.initial_dark_timer >= self.initial_dark_duration:
                ...
                self.activate_flash()  # Flash cuando se quite la oscuridad inicial
                self.dark_mode = False  # Empezar ciclos normales
                self.dark_timer = 0
            return  # No hacer nada más durante la oscuridad inicial
        
        # Ciclo de luz/oscuridad normal (después del segundo 20)
        self.cycle_timer += dt
        ...
        if self.dark_mode:
            ...
            if self.dark_timer >= self.light_duration:
                self.dark_mode = True
                self.dark_timer = 0
                self.activate_flash()  # Flash cuando entra en oscuridad
    ...
    def draw(self, surf):
        # Oscuridad inicial (menos oscura para poder ver)
        if self.initial_darkness:
            dark_overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            dark_overlay.fill((0, 0, 0, self.dark_alpha))  # 180 en lugar de 255
            surf.blit(dark_overlay, (0, 0))
            
           
        else:
            # Ciclos normales de oscuridad/luz
            if self.dark_mode:
                dark_overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
                dark_overlay.fill((0, 0, 0, 150))  # Oscuridad durante ciclos (menos intensa)
                surf.blit(dark_overlay, (0, 0))
                
        # Efecto flash (siempre visible cuando está activo)
        if self.active:
...
class IntroductionSystem:
    ...
    def start_intro_music(self):
        if not self.music_started and has_music:
            try:
                ...
                pygame.mixer.music.load(intro_music)  # Música de introducción
                pygame.mixer.music.set_volume(0.8)
    ...
    def start_level_music(self):
        if has_music:
            try:
                pygame.mixer.music.stop()
                pygame.mixer.music.load(nivel_music)  # Música del nivel
                pygame.mixer.music.set_volume(0.8)
                pygame.mixer.music.play(-1)
            except:
...
class Ghost:
    ...
    def update(self, dt, enemy_bullets, player):
        ...
        # Ataque cuerpo a cuerpo - CORREGIDO: todos los fantasmas tienen attack_cooldown
        self.attack_timer -= dt
        dist_to_player = math.sqrt((player.x - self.x)**2 + (player.y - self.y)**2)
        if dist_to_player < 50 and self.attack_timer <= 0:
            player.take_damage()
            self.attack_timer = self.attack_cooldown  # CORREGIDO: ya no da error
...
# Variables para control de oleadas de fantasmas - MODIFICADAS
ghost_spawn_timer = 0
...
while running:
    ...
    if not game_over and not level_cleared and fight_started and not results_system.active and not knockout_effect.freeze_game:
        ...
        flash_effect.update(dt)  # Actualizar efecto de flash
        ...
        # Generar fantasmas - SISTEMA MEJORADO DE DIFICULTAD
        ghost_spawn_timer += dt
        
        # NUEVO SISTEMA: Dificultad progresiva pero equilibrada
        if level_timer < 30:  # Primeros 30 segundos - Fácil
            current_spawn_interval = 2.0  # Fantasmas lentos
            spawn_chance = 0.8  # 80% de probabilidad de spawn
            max_ghosts = 5  # Máximo de fantasmas en pantalla
            
        elif level_timer < 90:  # Segundos 30-90 - Medio
            current_spawn_interval = 1.2
            spawn_chance = 0.9  # 90% de probabilidad
            max_ghosts = 8
            
        elif level_timer < 120:  # Segundos 90-120 - Difícil (pico de dificultad)
            current_spawn_interval = 0.8
            spawn_chance = 0.95  # 95% de probabilidad
            max_ghosts = 12
            
        else:  # Últimos 30 segundos - Más fácil para sobrevivir
            current_spawn_interval = 2.5  # Mucho más lento
            spawn_chance = 0.4  # Solo 40% de probabilidad
            max_ghosts = 6  # Menos fantasmas en pantalla
        
        # Verificar si podemos generar un nuevo fantasma
        if (ghost_spawn_timer >= current_spawn_interval and 
            ...
            # Determinar tipo de fantasma según progreso
            ...
            if level_timer < 30:
                ...
                if ghost_type_chance < 0.4:
                    ...
                    ghost_type = "charger"
            else:  # Últimos 30 segundos - menos fantasmas fuertes
        ...
        for ghost in ghosts[:]:
            ghost.update(dt, enemy_bullets, player)
    ...
    # Dibujar partículas de fantasmas
    ghost_particles.draw(screen)
    
    # Aplicar efectos de flash y oscuridad (¡IMPORTANTE! esto va al final)
